...
public class FromNineToTwelvePossibleCallTimeTgCallbackQueryHandlers implements TgCallbackQueryHandlers {

    private final PossibleCallTimeService possibleCallTimeService;

    @Override
    public Optional<ResponseDto> handle(UpdateDto dto) {
        return possibleCallTimeService.handle(dto, PossibleCallTime.FROM_NINE_TO_TWELVE);
    }

    @Override
    public CallbackTypes getCallbackType() {
        return CallbackTypes.FROM_NINE_TO_TWELVE;
    }
}
