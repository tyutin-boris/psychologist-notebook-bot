...
public class FromThreeToSixPossibleCallTimeTgCallbackQueryHandlers implements TgCallbackQueryHandlers {

    private final PossibleCallTimeService possibleCallTimeService;

    @Override
    public Optional<ResponseDto> handle(UpdateDto dto) {
        return possibleCallTimeService.handle(dto, PossibleCallTime.FROM_THREE_TO_SIX);
    }

    @Override
    public CallbackTypes getCallbackType() {
        return CallbackTypes.FROM_THREE_TO_SIX;
    }
}
