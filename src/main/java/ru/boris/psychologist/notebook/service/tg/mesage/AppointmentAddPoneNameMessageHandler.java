...
public class AppointmentAddPoneNameMessageHandler implements MessageHandler {
    ...
    public Optional<ResponseDto> handle(Long clientId, UpdateDto dto) {
        ...
        log.debug("Начало обработки запроса на добавления телефона. updateId: {}", updateId);

        makeAppointmentService.savePoneNumberForAppointment(dto);
        makeAnAppointmentService.saveAppointmentAddPossibleCallTime(clientId, updateId);
        return responseAppointmentService.getPossibleCallTime(dto);
    }

    @Override
    public ClientMessageStepType getStepType() {
        return ClientMessageStepType.MAKE_AN_APPOINTMENT_ADD_PHONE;
    }
}
