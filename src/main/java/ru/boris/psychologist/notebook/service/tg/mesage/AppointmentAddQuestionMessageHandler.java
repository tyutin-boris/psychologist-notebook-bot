...
public class AppointmentAddQuestionMessageHandler implements MessageHandler {
    ...
    public Optional<ResponseDto> handle(Long clientId, UpdateDto dto) {
        ...
        log.debug("Начало обработки запроса на добавление вопроса клиента. " +
                "updateId: {}, clientId: {}", updateId, clientId);

        makeAppointmentService.saveQuestionForAppointment(dto);
        makeAnAppointmentService.saveAppointmentAddEnd(clientId, updateId);
        return responseAppointmentService.getEndResponse(dto);
    }

    @Override
    public ClientMessageStepType getStepType() {
        return ClientMessageStepType.MAKE_AN_APPOINTMENT_ADD_QUESTION;
    }
}
